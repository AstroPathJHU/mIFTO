...
#' @param bin.estimate is the estimation of the bin to use if the FD
#'  choice fails
#' @return a data.frame with mids, density, and counts columns
#' @export
#'
mIFTO.create.histo <- function(data.in, bin.estimate) {
  ...
  hh1 = data.table::setnames(
    data.table::as.data.table(cut(data.in, Num_bins, labels = F)),'Bin.Num')
  hh2 = data.table::setnames(
    data.table::as.data.table(seq(1, Num_bins, by = 1)), 'Bin.Num')

  hh = dplyr::mutate(dplyr::summarize(
    dplyr::group_by(rbind(hh1,hh2),
                    Bin.Num), counts1 = dplyr::n()),counts = counts1 - 1,
